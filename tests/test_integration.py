...
# Since pyspark might not be installed in the environment where tests run (e.g. CI),
# we need to handle imports carefully or rely on the mocks set up in conftest.py.
# However, if we want to use specific classes from pyspark in our test logic (like SparkSession),
# we might need to mock them if the import fails.
...
class TestIntegration:
    ...
    def spark(self):
        ...
        spark.catalog._jcatalog = MagicMock()
        spark.catalog._jcatalog.databaseExists.return_value = True
        spark.catalog.listDatabases.return_value = [MagicMock(name="datamov_monitoring_db")]
    ...
    def test_engine_run(self, spark, tmp_path):
        ...
        with patch("datamov.core.engine.Engine.SparkManager") as MockSparkManager, \
            ...
            if isinstance(spark, MagicMock):
                 ...
                 # Verify that count() was called on the transformed dataframe (which determines if save_data is called)
